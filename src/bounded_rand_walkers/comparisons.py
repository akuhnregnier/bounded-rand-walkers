...
def get_binned_numerical_2D(
    ...
    np.abs((ft_xs[1] - ft_xs[0]) * (ft_ys[1] - ft_ys[0]))
    ...
    g_numerical, _, _ = np.histogram2d(
        *positions.T, bins=[x_edges, y_edges], normed=True
    )
    ...
    f_t_numerical, _, _ = np.histogram2d(
        *step_values.T, bins=[ft_xs, ft_ys], normed=True
    )

    rot_steps_data = rot_steps(positions.T)
    rot_probs, _, _ = np.histogram2d(
        rot_steps_data[0, :], rot_steps_data[1, :], bins=[ft_xs, ft_ys], normed=True
    )
...
def compare_1D(
    ...
    pickle_name = "1D_compare_data_{:}_{:}_{:}_{:.1e}.pickle".format(
        pdf_name, pdf_kwargs, nr_bins, num_samples
    )
    ...
    f_i_analytical = np.array(
        [pdf(np.array([x_coord])) for x_coord in step_bin_centres], dtype=np.float64
    )
    ...
    f_t_analytical /= np.sum(
        f_t_analytical * (step_bin_edges[1:] - step_bin_edges[:-1])
    )
    ...
    g_numerical, pos_bin_edges = np.histogram(
        positions, bins=pos_bin_edges, density=True
    )
    f_t_numerical, step_bin_edges = np.histogram(
        step_values, bins=step_bin_edges, density=True
    )
    ...
    f_i_numerical[shaper_mask] = f_t_numerical[shaper_mask] / shaper[shaper_mask]

    # normalise f_i_numerical
    # using the bins specified in ``step_bin_edges``
    f_i_numerical /= np.sum(f_i_numerical * (step_bin_edges[1:] - step_bin_edges[:-1]))
...
def compare_2D(
    ...
    pickle_name = "2D_compare_data_{:}_{:}_{:}_{:}_{:.1e}_+check.pickle".format(
        pdf_name, pdf_kwargs, bounds_name, nr_bins, num_samples
    )
    pickle_path = os.path.join(output_dir, pickle_name)
    if os.path.isfile(pickle_path) and load:
        with open(pickle_path, "rb") as f:
    ...
    np.abs((x_edges[1] - x_edges[0]) * (y_edges[1] - y_edges[0]))
    ...
    ft_rads = np.sqrt(ft_xcoords ** 2.0 + ft_ycoords ** 2.0)
    ...
    f_t_analytical[~ft_total_mask] = 0.0

    if bounds_name == "weird":
        shaper = get_weird_shaper(ft_x_values, ft_y_values, divisions=nr_bins)
    else:
    ...
    f_i_numerical[~shaper_mask] /= np.sum((f_i_numerical * ft_cell_area)[~shaper_mask])
    ...
    shaper_mask = ~np.isclose(shaper, 0)
    f_i_check[shaper_mask] = f_t_analytical[shaper_mask] / shaper[shaper_mask]

    # normalise f_i_check
    shaper_mask = np.isclose(shaper, 0)  # avoid dividing by 0
    f_i_check[~shaper_mask] /= np.sum((f_i_check * ft_cell_area)[~shaper_mask])
...
def compare_1D_plotting(
    ...
    # save all figures
    suffix = "{:} {:} {:} {:.1e}.png".format(pdf_name, pdf_kwargs, nr_bins, steps)
...
def compare_2D_plotting(
    ...
    if not np.all(np.isnan(g_numerical)):
        max_value = np.max(
            [
                np.max(g_analytical[~np.isnan(g_analytical)]),
                np.max(g_numerical[~np.isnan(g_numerical)]),
            ]
        )
        min_value = np.min(
            [
                np.min(g_analytical[~np.isnan(g_analytical)]),
    ...
    analytical_mesh = axes[0].pcolormesh(
        ...
        g_analytical[x_lims[0] : x_lims[1] + 1, y_lims[0] : y_lims[1] + 1].T,
    ...
    numerical_mesh = axes[1].pcolormesh(
        pos_x_edges[x_lims[0] : x_lims[1] + 2],
        pos_y_edges[y_lims[0] : y_lims[1] + 2],
        g_numerical[x_lims[0] : x_lims[1] + 1, y_lims[0] : y_lims[1] + 1].T,
        norm=colors.PowerNorm(gamma=2.0),
        vmin=min_value,
        vmax=max_value,
    )
    ...
    max_value = np.max(
        [
            np.max(f_t_analytical[~np.isnan(f_t_analytical)]),
            np.max(f_t_numerical[~np.isnan(f_t_numerical)]),
        ]
    )
    min_value = np.min(
        [
            np.min(f_t_analytical[~np.isnan(f_t_analytical)]),
    ...
    rot_probs_plot = ax.pcolormesh(
        ft_xs, ft_ys, rot_probs.T, norm=colors.PowerNorm(gamma=0.5)
    )
    fig3.colorbar(rot_probs_plot, label="P(x,y)")
    ax.hlines((0.0,), np.min(ft_xs), np.max(ft_xs), colors="b")
    ax.set_aspect("equal")
    ...
    plt.plot(avg_f_t_ana_radii, avg_f_t_analytical, label="Analytical", zorder=5)
    plt.plot(
        avg_f_t_num_radii,
        avg_f_t_numerical,
        label="Numerical",
    ...
    plt.plot(avg_f_i_ana_radii, avg_f_i_analytical, label="Analytical", zorder=4)
    plt.plot(
        avg_f_i_num_radii,
        avg_f_i_numerical,
        label="Numerical",
    ...
    # save all figures
    suffix = "{:} {:} {:} {:} {:.1e}.png".format(
        pdf_name, pdf_kwargs, bounds_name, nr_bins, steps
    )
...
if __name__ == "__main__":
    ...
    if TWO_D:
        ...
        pdfs_args_2D = [
            (Funky, "funky", {"centre": np.array((0.0, 0.0)), "width": 2.0}),
            (Gaussian, "gauss", {"centre": np.array((0.0, 0.0)), "width": 0.8}),
        ]
