...
double tophat(const dvect &x, dvect &grad, void *my_func_data)
{
    ...
    // print(pos);
    if (xt::all(
            xt::sqrt(xt::sum(xt::pow(
                        (pos - xt::view(centre, xt::range(0, x.size()))),
                         2.)
            )) < (width / 2.))) {
        return 1.;
    } else {
...
double arbitrary(const dvect &x, dvect &grad, void *my_func_data)
{
    ...
    // print("attempting");
    // print(pos);
    auto r = -xt::sum(xt::pow((pos - xt::view(centre, xt::range(0, x.size()))), 2.));
    return r[0];
}


xt::xarray<double> find_maximum(nlopt::vfunc func, std::vector<double> &x, std::vector<double> &lb, std::vector<double> &ub, void * my_func_data) {
    /* func: pdf
     * x: initial guess
     * lb: lower bound
     * ub: upper bound
