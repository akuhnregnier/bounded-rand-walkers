...
double find_maximum(nlopt::vfunc func, dvect &x, dvect &lb, dvect &ub, void * my_func_data) {
     ...
     */
    nlopt::opt opt;
    opt = nlopt::opt(nlopt::LN_COBYLA, x.size()); /* algorithm and dimensionality */
    ...
    // nlopt_destroy(opt);
    dxarray max_pos;
    max_pos = xt::zeros<double>({x.size(),});
...
struct interp {
    ...
    interp(dvect& grid, dvect& f_values) {
        ...
        // now assign this to the pointer so it can be used later
        interp_ML_ptr = new InterpMultilinear<1, double>(grid_iter_list.begin(), grid_sizes.begin(), f_values.data(), f_values.data() + num_elements);
    }

    template <class array_like>
    double operator() (const array_like& args) {
        // interpolate one value
        return interp_ML_ptr -> interp(args.begin());
    }
    ~interp() {
        delete interp_ML_ptr;
    }
};
...
class Sampler1D: public SamplerBase {
    public:
        ...
        Sampler1D(int blocks, double pdf(const dvect &, dvect &, void *), pdf_data* func_data) {
            print("4 arg init called");
            func_data_store = func_data;
            dvect centre_vect = func_data_store -> centre;
            std::vector<std::size_t> centre_shape = { centre_vect.size(), };

            dxarray centre = xt::adapt(centre_vect, centre_shape);
            double width = func_data -> width;

            pdf_values = xt::zeros<double>({blocks,});
            max_box_values = xt::zeros<double>({blocks,});
            ...
            for (int i=0; i<blocks; ++i) {
                ...
                for (int j=0; j<starts.size(); ++j) {
                    ...
                    found_max = find_maximum(pdf, starts[j], lower_edge, upper_edge, func_data);
            ...
            double max_discrete_cdf = *std::max_element(discrete_cdf.begin(), discrete_cdf.end());
        ...
        dvect sample(const dvect& position, double pdf(const dvect &, dvect &, void *)) {
            ...
            do {
                ...
                // std::uniform_real_distribution<double> dis(min_prob, max_prob); // uniform, unbiased
                // dvect prob = {dis(gen)};
                ...
                // std::uniform_real_distribution<double> dis2(0., 1.); // uniform, unbiased
                // prob2 = dis2(gen);
...
class Sampler2D: public SamplerBase {
    public:
        ...
        Sampler2D(int blocks, double pdf(const dvect &, dvect &, void *), pdf_data* func_data) {
            if (VERBOSE) {
                print("4 arg init called");
            }
            // make sure that the vectors do not have to re-allocate, which does not work with the 'interp' struct apparently
            ...
            edges = xt::linspace<double>(xt::index_view(bounds, {{0, 0}})[0], xt::index_view(bounds, {{1, 0}})[0], blocks+1);
            centres = (xt::view(edges, xt::range(1, blocks + 1))
                       + xt::view(edges, xt::range(0, blocks))) / 2.;
            ...
            auto first_summed_values = xt::sum(max_box_values, { 1, });

            // now use these max values to define the required interpolators
            first_discrete_cdf = cumsum(first_summed_values);
            double max_discrete_cdf = *std::max_element(first_discrete_cdf.begin(), first_discrete_cdf.end());
            ...
            // as opposed to the 1D case, we now need to add more interpolators for the 2nd dimension

            second_discrete_cdf = xt::zeros<double>({ blocks + 1, });
            ...
            for (size_t row(0); row<blocks; ++row) {
                ...
                xt::view(second_discrete_cdf, xt::range(1, xt::placeholders::_)) = cumsum(row_data);

                double max_second_discrete_cdf = *std::max_element(second_discrete_cdf.begin(), second_discrete_cdf.end());
                if (VERBOSE) {
                    print("max");
                ...
                // can re-use the `edges_vect` from above, since the edges are symmetric in both directions!
                //
        ...
        dvect sample(const dvect& pos, double pdf(const dvect &, dvect &, void *)) {
            ...
            // axes_step_bounds = xt::clip(axes_step_bounds, xt::amin(bounds, {0}), xt::amax(bounds, {0}));
            ...
            do {
                ...
                for (size_t i=0; i<edges.size(); ++i) {
                    if (coord[0] < edges[i]) {
                        ...
                        break;  // break out of the for loop as the desired index has been found
                    }
                }

                // repeat the generation above for the second dimension, using the `first_max_val_index`
                // in doing so, append to `coord` using push_back such that the
                // required 2 coordinates are generated
                ...
                auto max_box_val = xt::index_view(max_box_values, {{first_max_val_index,
...
auto cumsum(const arr_type& arr) {
    auto cum_sum = arr;
...
void testing_2d() {
    ...
    print(test);
    auto edges = xt::linspace<double>(xt::index_view(bounds, {{0, 0}})[0], xt::index_view(bounds, {{1, 0}})[0], blocks+1);
    print(edges);
    print(edges.size());
    ...
    dxarray bounds2 =
        {{ -2, -2 },
         { 2, 2 }};
    dxarray axes_step_bounds =
        {{ -1 + 0.1, 1 - 0.2 },
         { -1 + 0.1, 1 - 0.2 }};

    auto clipr = xt::clip(axes_step_bounds, xt::amin(bounds2, {0}), xt::amax(bounds2, {0}));
